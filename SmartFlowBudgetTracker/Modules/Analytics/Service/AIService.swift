...
final class AIService: ObservableObject {
    ...
    func generateInsight(for items: [LedgerItem]) async -> Insight {
        ...
        case .mock:
            ...
            return Insight(
                message: "You're trending \(total >= 0 ? "positive" : "negative") this month.",
                confidence: 0.65
            )
        case .local:
            try? await Task.sleep(nanoseconds: 120_000_000)
            return Insight(
                message: "Local model suggests reducing discretionary spending by 10%.",
                confidence: 0.54
            )
        case .remote:
            try? await Task.sleep(nanoseconds: 250_000_000)
            return Insight(
                message: "GPT recommends moving excess cash into a high-yield savings goal.",
                confidence: 0.72
            )
        }
    }
